...
>(
  (
    ...
    if (isMobile) {
        return (
            <div className={cn("bg-sidebar text-sidebar-foreground", className)} ref={ref} {...props}>
                {children}
            </div>
        );
    }
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      className={cn(
        "flex-1 min-h-0 overflow-y-auto overflow-x-hidden p-2 group-data-[collapsible=icon]:overflow-hidden",
        className
      )}
      {...props}
    />
